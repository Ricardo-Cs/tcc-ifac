...
function conflito(severidade: SeveridadeConflito, tipo: TipoConflito): Conflito {
    return {
        tipo,
        severidade,
...
describe('chaveDoAceite', () => {
    it('rejeita aceitar um conflito FORTE', () => {
        const forte = conflito(SeveridadeConflito.FORTE, TipoConflito.TURMA_DUPLICADA);
    ...
    it('devolve a chave para um conflito FRACO', () => {
        const fraco = conflito(SeveridadeConflito.FRACO, TipoConflito.HORARIO_NAO_PREFERIDO);
